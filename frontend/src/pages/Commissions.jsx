...
import {
  ...
  DollarSign, Calendar, Hourglass, Sparkles, LogIn
} from 'lucide-react';
import { toast } from 'sonner';
import {
  fetchCommissions, fetchCommissionStats, deleteCommission
} from '../api';
...
const TOKEN_KEY = 'veri_admin_token'; // eslint-disable-line no-unused-vars
...
// ---------------- Admin Login Modal (Google + password fallback) ----------------
          ...
          {canEdit ? (
            <>
              ...
              <button onClick={logout} data-testid="admin-logout-btn"
                className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/5 border border-white/10 text-[#B1EDE8] text-sm hover:bg-white/10 transition-all">
                {user?.picture && <img src={user.picture} alt="" className="w-5 h-5 rounded-full" />}
                <span className="hidden sm:inline">{user?.name ? `Sign out (${user.name.split(' ')[0]})` : 'Lock'}</span>
